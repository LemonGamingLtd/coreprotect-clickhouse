    ...
    static void process(Statement statement, PreparedStatement preparedStmt, int batchCount, int processId, int id, String user, Object object, Material blockType, int blockData, String overrideData) {
        if (object instanceof QueuedBlockState queuedBlock) {
            ...
            if (blockLists.get(id) != null) {
                ...
                BlockBreakLogger.log(preparedStmt, batchCount, user, queuedBlock.location(), MaterialUtils.getBlockId(blockType), blockData, queuedBlock.meta(), queuedBlock.blockData(), overrideData);
            }
        }
